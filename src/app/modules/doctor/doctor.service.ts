...
const sendVerificationEmail = async (data: Doctor) => {
    console.log('Starting email verification process for:', data.email);
    ...
    console.log('Verification data created:', verficationData);
    if (verficationData) {
        const pathName = path.join(__dirname, '../../../../template/verify.html')
        console.log('Template path:', pathName);
        const obj = {link: url};
        const subject = "Email Verification"
        const toMail = data.email;
        try{
            console.log('Attempting to send email to:', toMail);
